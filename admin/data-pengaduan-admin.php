    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 p-4 sm:p-6 md:p-8 lg:p-10 text-white">
            ...
            <div class="mb-4 sm:mb-6">
                <form method="GET" class="flex flex-col sm:flex-row gap-2 sm:gap-2">
                    <input 
                        type="text" 
                        name="search" 
                        value="<?= htmlspecialchars($search) ?>"
                        placeholder="Cari judul, deskripsi, lokasi..." 
                        class="flex-1 px-3 sm:px-4 py-2 rounded-lg bg-[#1a2422] text-white border border-[#42506a] text-sm sm:text-base
                               focus:outline-none focus:border-[#a4c6c3] placeholder-gray-400"
                    >
                    <select 
                        name="status"
                        class="px-3 sm:px-4 py-2 rounded-lg bg-[#1a2422] text-white border border-[#42506a] text-sm sm:text-base
                    ...
                    <button 
                        type="submit" 
                        class="px-4 sm:px-6 py-2 bg-[#42506a] text-white rounded-lg hover:bg-[#0b1110] text-sm sm:text-base
                    ...
                    <?php if ($search !== '' || $status_filter !== ''): ?>
                        <a 
                            ...
                            class="px-4 sm:px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm sm:text-base
                ...
                <table class="w-full text-xs sm:text-sm text-left text-[#0b1110]">
                    <thead class="bg-[#42506a] text-white sticky top-0">
                        <tr>
                            ...
                            <th class="px-2 sm:px-4 md:px-6 py-2 sm:py-3 hidden sm:table-cell">NIS</th>
                    ...
                    <tbody>
                        ...
                        <?php if ($total_pengaduan > 0): ?>
                            <?php foreach ($pengaduan_list as $data) : ?>
                                <tr class="border-b hover:bg-[#a4c6c3] transition duration-200">
                                    ...
                                    <td class="px-2 sm:px-4 md:px-6 py-2 sm:py-4 hidden sm:table-cell text-xs">
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="9" class="text-center py-6 text-gray-500 text-xs sm:text-base">
